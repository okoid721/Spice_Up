...
function BreakfastMap() {
  ...
  return (
    <div className="w-full h-screen grid grid-cols-3 gap-16 mb-8 mt-8">
      {breakfast.map((data, i) => {
        return (
          <div
            key={i}
            className="w-[300px] h-[500px] shadow-lg flex flex-col rounded-[10px] items-center flex-wrap"
          >
            <img
              src={data.image}
              alt={data.name}
            ...
            <p className=" px-4">
              <TruncateText text={data.ingredients} maxLength={100} />
            </p>
          </div>
        );
      })}
